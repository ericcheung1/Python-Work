...
ns_tables = nba_soup.find_all('table') #### fix name
ns_table_1 = ns_tables[0].find_all('tr')

ns_parsed_rows = [parse_row(row) for row in ns_table_1[1:]]

per_game = DataFrame(ns_parsed_rows)
per_game.columns = [str(x.string) for x in ns_table_1[0].find_all('th')]
per_game.drop('Awards', axis=1, inplace=True)
per_game.head()
...
ads_tables = advanced_soup.find_all('table') #### fix name
ads_table_1 = ads_tables[0].find_all('tr')

ads_parsed_rows = [parse_row(row) for row in ads_table_1[1:]]

advanced_stats = DataFrame(ads_parsed_rows)
advanced_stats.columns = [str(x.string) for x in ads_table_1[0].find_all('th')]
...
ss_table_1_row_1 = sal_tables[0] ####### fix names 

header_row = ss_table_1_row_1.find('thead').find('tr')
header = [header.text.strip() for header in header_row.find_all('td')]

ss_table_1_rows = ss_table_1_row_1.find_all('tr')[1:]
